...
impl Process {
    pub(crate) fn run(exe_path: &str, args: &[&str]) -> Result<Self> {
    ...
    pub(crate) fn attach(pid: i32) -> Result<Self> {
    ...
    pub(crate) fn print_map_infos(&self) {
        for map in &self.maps {
            println!("{}", map);
        }
    }

    pub(crate) fn get_random_rw_memory(&self) -> Result<u64> {
        for map in &self.maps {
    ...
    pub(crate) fn get_program_name(&self) -> Result<String> {
    ...
    pub(crate) fn get_program_name_from_pid(pid: Pid) -> Result<String> {
    ...
    pub(crate) fn get_base_addr_from_map(maps: &[Map], pid: Pid) -> Option<u64> {
